...
const FeedPage = () => {
  ...
  const fetchFeed = useCallback(async (pageNum, activeTab, isLoadMore = false) => {
    if (pageNum === 1) setLoading(true);
    else setLoadingMore(true);

    try {
      const { data } = await postApi.getFeed({ tab: activeTab, page: pageNum, limit: 10 });
      if (isLoadMore) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--color-border)] pb-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2 font-[Outfit]">
            Ana Akış <Sparkles size={20} className="text-indigo-400" />
          </h1>
        ...
        {/* Tab switchers */}
        <div className="flex bg-[var(--color-surface-2)] p-1 rounded-xl border border-[var(--color-border)] self-start md:self-center">
          <button
